...
1. List plugin packages and standalone plugin assemblies from the connected environment using OData. For packages, show `pluginpackageid`, `name`, `uniquename`, `version`, `package_name`, `fileid`, state, status, and managed identity reference when available. Exclude plugin assemblies associated with a plugin package by filtering `pluginassemblies` where `_packageid_value eq null`. Both list requests select `_managedidentityid_value` and expand `managedidentityid` with `managedidentityid`, `name`, `applicationid`, `tenantid`, `credentialsource`, `subjectscope`, `version`, `statecode`, and `ismanaged`.
2. Display packages and assemblies in separate tabs with client-side pagination. Show whether each component belongs to a managed or unmanaged solution, show a sortable Managed identity column with the related identity name, and provide Inspect and Export actions for packages and assemblies. The Managed identity column sorts by identity name and then by managed identity ID. When a component references a managed identity that the caller cannot read, the column shows `Restricted`; when no identity is associated, it shows a placeholder. Assemblies without stored binary content remain visible, but inspection and export must report that special or externally hosted assemblies cannot be processed.
...
11. Parse the CMS envelope and the signer certificate with `PKI.js` and show the issuer DN, subject DN, serial number, validity, fingerprint, certificate category, and an embedded CMS certificate chain tree from root to signer. Render issuer and subject DNs as readable X.509 attributes while preserving the raw values used for identity calculations. Distinguished names are serialized as `attributeType=value` pairs joined by `/`, using X.500 attribute type names and falling back to the raw OID for unknown attributes. Show the chain and field/value details for a user-selected certificate in a dedicated in-app popup on request. Select the signer certificate by the CMS signer identifier, matching either the issuer-and-serial or the subject-key-identifier signer reference, rather than the first certificate in the embedded set.
...
15. Show the associated managed identity name in the inspection summary next to the Signature and Signer values for a component read from Dataverse. When the component has no associated identity, show that no managed identity is associated with the package or assembly; when the related record could not be read, show that instead. When an identity is available, offer a `View managed identity details` action that opens a dedicated in-app popup mirroring the certificate-details popup. The popup shows the identity name, application ID, tenant ID, credential source, subject scope, federated credential subject version, status, and managed/unmanaged type, mapping option-set values to readable labels and rendering unrecognized values as `Unknown (<value>)`. Provide a Toolbox clipboard action for the application ID and tenant ID, and close the popup with Escape or a click outside it. Local file inspections do not show the managed identity row or action.
16. Prefill the Tenant ID setting from the inspected component's managed identity when the field is still empty, before falling back to `RetrieveCurrentOrganization`. A manual edit always wins. Warn in the managed identity details popup and the event log when the managed identity tenant differs from the tenant used to compute the subject identifier, and when the identity's federated credential subject version is not version 2.
17. Produce the managed identity version 2 subject identifier:
   - Trusted certificate: issuer and subject SHA-256 hashes over the exact UTF-8 DN strings, encoded as Base64URL.
   - Self-signed certificate: SHA-256 hash over certificate DER bytes, encoded as lowercase hexadecimal.
   - Tenant ID: convert the GUID to .NET `Guid.ToByteArray()` byte order before Base64URL encoding.
18. For a signed package or assembly, retrieve `TenantId` and `EnvironmentId` using the `RetrieveCurrentOrganization` function, while allowing manual edits, then combine those values with cloud configuration to compute the Step 3 federated credential issuer and version 2 subject identifier.
19. Mark the row that produced the current inspection with a green checkmark in its Inspect action; while hovering that checkmark, show the inspect icon and indicate that the inspection can be run again. Mark the Inspect Local Package button when the current result came from a local file.
20. Refreshing packages must clear the current inspection result, inspected component indicator, inspected managed identity record, generated identity result, and certificate-details popup state.

## Data Access

The Plugin Package entity set is `pluginpackages`. The list request excludes package payload fields. The managed identity entity set is `managedidentities`; its data is read through the `managedidentityid` navigation property on `pluginpackage` and `pluginassembly`, so no separate request is issued. When the caller lacks read access to `managedidentity` and the expanded query fails, the same query is retried without the `$expand` clause so the component lists still load. Package inspection uses `InitializeFileBlocksDownload` with a `pluginpackage` target and `FileAttributeName` set to `package`, then decodes and assembles each `DownloadBlock` Base64 payload in memory. `pluginassembly.content` is not a Dataverse File column and cannot use `InitializeFileBlocksDownload`; assembly inspection retrieves the `content_binary` property first and falls back to the Base64 `content` property. Solution listing queries solution metadata and the publisher navigation property, while component membership is resolved through `solutioncomponents`. Browser `fetch` is not used.

## Non-Goals

- Updating plugin packages.
- Creating, updating, or associating managed identity records.
